...
def run():
    ...
    if st.button('add input'):
        ...
        input_41.tail(1).to_csv('log_42.csv', mode='a', index = False, header = False)

    kasus = st.text_input(f'Apakah ada kasus di KLHK 42?')
    ...
    if st.button('add output'):
        ...
        hasil_41.tail(1).to_csv('log_hasil_42.csv', mode='a', index=False, header=False)
