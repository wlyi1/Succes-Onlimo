...
def run():
    ...
    kasus = st.text_input(f'Apakah ada kasus di KLHK 43?')

    if st.button('add'):
        get_data_input().append({'Tanggal Aksi': tgl_aksi, 'Aksi':kasus})
        input_41 = pd.DataFrame(get_data_input())
        input_41.tail(1).to_csv('log_43.csv', mode='a', index = False, header = False)
    ...
    if st.button('add output'):
        ...
        hasil_41.tail(1).to_csv('log_hasil_43.csv', mode='a', index=False, header=False)
